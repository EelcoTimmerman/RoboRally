...
public class Slow extends Square{
    ...
    public Slow(String walls){
        super(walls);
    }

    public Slow(Direction direction){
        this.movementDirection = direction;
    }

    public Slow(Direction direction, String walls){
        super(walls);
        this.movementDirection = direction;
    }
    ...
    public String getType(){
        return "Slow" + this.movementDirection;
    }
    ...
    public static void addRobotsToSlowConveyorbeltList(Board board, List<Robot> robots){
        ...
        for(Robot robot : robots){
            ...
            if(currentPosition instanceof Slow & !(robotsOnSlowConveyorbelt.contains(robot))){
    ...
    private boolean moveRobotInDirectionIfPossible(Robot robot, Direction direction, Board board, List<Robot> otherRobots){
        ...
        Slow currentPosition = (Slow) board.getSquare(robot.getXCoordinate(), robot.getYCoordinate());
            ...
            if(!respawnRobotIfNecessary(robot, board)){
                ...
                for(Robot otherRobot : otherRobots){
                    ...
                    if(otherRobot.isAt(robot.getXCoordinate(), robot.getYCoordinate()) && (otherRobot != robot & destination instanceof SlowConveyorbelt)){
                        ...
                        Slow destinationBelt = (Slow) destination;
    ...
    private Map<Robot, Square> mapExpectedDestinations(Board board, List<Robot> robots){
        Map<Robot, Square> expectedDestinationRobots = new HashMap<Robot, Square>();
        for(Robot robot : robots){
            Square currentRobotPosition = board.getSquare(robot.getXCoordinate(), robot.getYCoordinate());
            if(currentRobotPosition instanceof Slow){
                Square robotDestination = getDestination(robot.getXCoordinate(), robot.getYCoordinate(), board);
                expectedDestinationRobots.put(robot, robotDestination);
            }
        }
        ...
        // Square currentRobotPosition = board.getSquare(robot.getXCoordinate(), robot.getYCoordinate());
        // Square currentRobotDestination = getDestination(robot.getXCoordinate(), robot.getYCoordinate(), board);
        // Direction directionConveyorBelt = ((Slow) currentRobotPosition).getMovementDirection();

        // for(Robot otherRobot : robots){
        //     Square otherRobotPosition = board.getSquare(otherRobot.getXCoordinate(), otherRobot.getYCoordinate());
        //     Square otherRobotDestination = getDestination(otherRobot.getXCoordinate(), otherRobot.getYCoordinate(), board);
        //     if(otherRobotPosition == currentRobotDestination){
        //         if(!(otherRobotPosition instanceof Slow)){
        //             for(Robot pushedRobot : robots){
        //                 Square pushedRobotDestination = expectedPushedDestination(pushedRobot, board, directionConveyorBelt);
        //                 if(otherRobotDestination == pushedRobotDestination){
        //                 }
        //             }
        //         }
        //         else{
        ...
        //     }
        // }
        return expectedDestinationRobots;
    }

    private Square expectedPushedDestination(Robot robot, Board board, Direction direction){
        Square expectedDestionation;
        ...
        expectedDestionation = getDestination(xCoordinate, yCoordinate, board);
        return expectedDestionation;
    }
    ...
    private String turnRobotIfNecessary(Robot robot, Board board){
        ...
        if(destination instanceof Slow){
            Slow destinationBelt = (Slow) destination;
    ...
    private Square getDestination(int xCoordinate, int yCoordinate, Board board){
        Slow origin = (Slow) board.getSquare(xCoordinate, yCoordinate);
